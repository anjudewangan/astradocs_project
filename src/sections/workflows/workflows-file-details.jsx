...
import { WorkflowsInvitedItem } from './workflows-invited-item';

export function WorkflowsFileDetails({ item, open, onClose, onDelete, ...other }) {
          ...
          <DialogContent>
            ...
            {hasShared && (
              <Box component="ul" sx={{ pl: 2, pr: 1 }}>
                {shared.map((person) => (
                  <WorkflowsInvitedItem key={person.id} person={person} />
                ))}
              </Box>
            )}
          </DialogContent>
